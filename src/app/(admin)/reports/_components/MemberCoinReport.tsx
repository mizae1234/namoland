...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
    Purchase: { label: "ซื้อเหรียญ", color: "text-emerald-600 bg-emerald-50" },
    Class: { label: "เข้าคลาส", color: "text-violet-600 bg-violet-50" },
    "Adjust+": { label: "ปรับเพิ่ม", color: "text-blue-600 bg-blue-50" },
    "Adjust-": { label: "ปรับลด", color: "text-orange-600 bg-orange-50" },
    Expired: { label: "หมดอายุ", color: "text-red-600 bg-red-50" },
    "BOOK RENTAL": { label: "ยืมหนังสือ", color: "text-amber-600 bg-amber-50" },
    "BOOK DEPOSIT": { label: "มัดจำหนังสือ", color: "text-amber-600 bg-amber-50" },
    "BORROW FEE": { label: "ค่ายืม", color: "text-amber-600 bg-amber-50" },
    "LATE FEE": { label: "ค่าเลท", color: "text-red-500 bg-red-50" },
};

function getTypeInfo(type: string) {
    return TYPE_LABELS[type] || { label: type, color: "text-gray-600 bg-gray-50" };
}
        ...
        <div>
            ...
            <Card className="mb-6">
                <div className="relative">
                    <label className="text-sm font-medium text-[#3d405b]/60 block mb-2">
                        ค้นหาสมาชิก (ชื่อผู้ปกครอง, เบอร์โทร, หรือชื่อเด็ก)
                    </label>
                    <div className="relative">
            ...
            {loadingReport && (
                <div className="text-center py-12">
                    ...
                    <p className="text-sm text-[#3d405b]/40">กำลังโหลดข้อมูล...</p>
                </div>
            )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                        <Card className="!bg-emerald-50 border border-emerald-200">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs font-medium text-emerald-600/70">เหรียญเข้า (ซื้อ)</span>
                            </div>
                        ...
                        <Card className="!bg-violet-50 border border-violet-200">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs font-medium text-violet-600/70">เหรียญออก (ใช้)</span>
                            </div>
                        ...
                        <Card className="!bg-amber-50 border border-amber-200">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs font-medium text-amber-600/70">คงเหลือ</span>
                            </div>
                        ...
                        <Card className="!bg-sky-50 border border-sky-200">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="text-xs font-medium text-sky-600/70">💰 ยอดเงินซื้อ</span>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="bg-[#f4f1de]/50 border-b border-[#d1cce7]/20 text-left">
                                            <th className="py-2.5 px-4 font-semibold text-[#3d405b]/60 text-xs">วันที่</th>
                                            <th className="py-2.5 px-4 font-semibold text-[#3d405b]/60 text-xs">ประเภท</th>
                                            <th className="py-2.5 px-4 font-semibold text-[#3d405b]/60 text-xs">รายละเอียด</th>
                                            <th className="py-2.5 px-3 font-semibold text-[#3d405b]/60 text-xs text-right">เหรียญเข้า</th>
                                            <th className="py-2.5 px-3 font-semibold text-[#3d405b]/60 text-xs text-right">เหรียญออก</th>
                                            <th className="py-2.5 px-3 font-semibold text-[#3d405b]/60 text-xs text-right">คงเหลือ</th>
                                            <th className="py-2.5 px-3 font-semibold text-[#3d405b]/60 text-xs text-right">จำนวนเงิน</th>
                                        </tr>
                                    </thead>
                                    ...
                                    <tfoot>
                                        <tr className="bg-[#f4f1de]/50 border-t-2 border-[#d1cce7]/30 font-semibold">
                                            <td colSpan={3} className="py-2.5 px-4 text-xs text-[#3d405b]/60">รวม</td>
            ...
            {!reportData && !loadingReport && (
                <div className="text-center py-16">
                    ...
                    <p className="text-[#3d405b]/40 text-sm">เลือกสมาชิกเพื่อดูรายงานเหรียญ</p>
                </div>
            )}
        </div>
    );
