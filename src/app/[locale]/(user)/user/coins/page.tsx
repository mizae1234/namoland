...
const TOPUP_STATUS_MAP: Record<string, { icon: typeof Clock; className: string }> = {
    PENDING: { icon: Clock, className: "text-amber-500 bg-amber-50" },
    APPROVED: { icon: CheckCircle, className: "text-emerald-500 bg-emerald-50" },
    REJECTED: { icon: XCircle, className: "text-red-500 bg-red-50" },
};

export default async function UserCoinsPage() {
    const t = await getTranslations("UserCoins");
    const localeString = await getLocale();
    const dateLocale = localeString === "en" ? enUS : th;
        ...
        <div className="p-4">
            ...
            <div className="bg-gradient-to-r from-amber-400 to-orange-400 rounded-2xl p-6 text-white mb-4 shadow-lg">
                <div className="flex items-center gap-2 mb-1">
                    <Coins size={20} />
                    <span className="text-amber-100 text-sm">{t("title")}</span>
                </div>
                ...
                {latestExpiry && totalCoins > 0 && (
                    <p className={`text-xs mt-2 ${daysLeft !== null && daysLeft <= 7 ? "text-white font-semibold" : "text-white/60"}`}>
                        {t("expiryPrefix")} {format(latestExpiry, "d MMM yyyy", { locale: dateLocale })}
                        {daysLeft !== null && daysLeft <= 7 && (
                            <span> ({daysLeft <= 0 ? t("expired") : t("expireDays", { days: daysLeft })})</span>
                        )}
                    </p>
                )}
            </div>
            ...
            {pendingRequests.length > 0 && (
                <div className="mb-6">
                    <h2 className="font-semibold text-[#3d405b] mb-3">{t("pendingRequestsTitle")}</h2>
                            ...
                            return (
                                <div key={req.id} className={`rounded-xl p-4 border ${status.className}`}>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-xs">{t(`status.${req.status}`)}</span>
                                    </div>
                                    <p className="text-xs mt-1 opacity-70">
                                        {format(new Date(req.createdAt), "d MMM yy HH:mm", { locale: dateLocale })}
                                    </p>
                                </div>
                            );
                        })}
            ...
            <h2 className="font-semibold text-[#3d405b] mb-3">{t("historyTitle")}</h2>
            {(() => {
                ...
                return (
                    <div className="bg-white rounded-xl border border-[#d1cce7]/20 divide-y divide-[#d1cce7]/15 mb-6">
                        {history.map((item) => {
                            const statusConfig = {
                                APPROVED: { labelKey: "APPROVED_SHORT", className: "text-emerald-500 bg-emerald-50", Icon: CheckCircle },
                                PENDING: { labelKey: "PENDING", className: "text-amber-500 bg-amber-50", Icon: Clock },
                                REJECTED: { labelKey: "REJECTED", className: "text-red-500 bg-red-50", Icon: XCircle },
                            };
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-[#3d405b]/40">
                                                {format(item.date, "d MMM yy HH:mm", { locale: dateLocale })}
                                                {item.source === "admin" && ` · ${t("adminTopUp")}`}
                                            </p>
                                        </div>
                                    </div>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${config.className}`}>{t(`status.${config.labelKey}`)}</span>
                                </div>
                            );
                        })}
                    </div>
            ...
            {/* Recent Transactions */}
            <h2 className="font-semibold text-[#3d405b] mb-3">{t("recentTransactionsTitle")}</h2>
            <div className="bg-white rounded-xl border border-[#d1cce7]/20 divide-y divide-[#d1cce7]/15">
                {packages.flatMap((p) => p.transactions).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 10).map((tx) => (
                    <div key={tx.id} className="px-4 py-3 flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-[#3d405b]/40">
                                {format(new Date(tx.createdAt), "d MMM yy", { locale: dateLocale })}
                            </p>
                        </div>
                ...
                {packages.flatMap((p) => p.transactions).length === 0 && (
                    <div className="px-4 py-6 text-center text-[#3d405b]/40 text-sm">{t("emptyTransactions")}</div>
                )}
            </div>
        </div>
    );
