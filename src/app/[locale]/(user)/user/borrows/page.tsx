...
// Initial config without labels
const STATUS_CONFIG: Record<string, { icon: typeof Clock; className: string; bgClass: string }> = {
    RESERVED: { icon: Clock, className: "text-[#a16b9f]", bgClass: "bg-[#a16b9f]/10" },
    BORROWED: { icon: BookOpen, className: "text-[#609279]", bgClass: "bg-[#81b29a]/10" },
    RETURNED: { icon: CheckCircle2, className: "text-emerald-600", bgClass: "bg-emerald-50" },
    OVERDUE: { icon: AlertCircle, className: "text-red-500", bgClass: "bg-red-50" },
    FORFEITED: { icon: AlertCircle, className: "text-[#3d405b]/50", bgClass: "bg-[#3d405b]/5" },
    CANCELLED: { icon: XCircle, className: "text-orange-500", bgClass: "bg-orange-50" },
};

export default async function UserBorrowHistoryPage() {
    const t = await getTranslations("UserBorrows");
        ...
        <div className="p-4 pb-24">
            ...
            {borrowRecords.length === 0 ? (
                ...
                <>
                    ...
                    {active.length > 0 && (
                        <div className="mb-6">
                            <h2 className="text-sm font-semibold text-[#3d405b]/60 mb-3">{t("activeTitle")}</h2>
                                        ...
                                        <div key={record.id} className="bg-white rounded-2xl border border-[#d1cce7]/20 overflow-hidden">
                                            ...
                                            <div className={`px-4 py-3 flex items-center justify-between ${config.bgClass}`}>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-sm font-medium ${config.className}`}>{statusLabel}</span>
                                                </div>
                                            ...
                                            <div className="px-4 py-3 bg-[#f4f1de]/30 border-t border-[#d1cce7]/10">
                                                <div className="flex items-center justify-between text-xs">
                                                    <div className="flex items-center gap-1.5 text-[#3d405b]/50">
                                                        <Calendar size={12} />
                                                        <span>{t("labels.borrowDate")}: {format(new Date(record.borrowDate), "d MMM yyyy", { locale: dateLocale })}</span>
                                                    </div>
                                                    <div className={`flex items-center gap-1.5 font-medium ${isOverdue ? "text-red-500" : "text-[#3d405b]/60"}`}>
                                                        <Clock size={12} />
                                                        <span>{t("labels.dueDate")}: {format(new Date(record.dueDate), "d MMM yyyy", { locale: dateLocale })}</span>
                                                    </div>
                                                </div>
                                                {/* Coins */}
                                                <div className="flex items-center gap-3 mt-2 text-xs text-[#3d405b]/40">
                                                    <span className="flex items-center gap-1">
                                                        <Coins size={11} />
                                                        {t("labels.rentalCoins")}: {record.rentalCoins}
                                                    </span>
                                                    {record.depositCoins > 0 && (
                                                        <span>{t("labels.depositCoins")}: {record.depositCoins}</span>
                                                    )}
                                                </div>
                                            </div>
                    ...
                    {completed.length > 0 && (
                        <div>
                            <h2 className="text-sm font-semibold text-[#3d405b]/60 mb-3">{t("pastTitle")}</h2>
                                    ...
                                    return (
                                        <div key={record.id} className="bg-white rounded-xl p-4 border border-[#d1cce7]/20">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 text-xs text-[#3d405b]/40">
                                                <div>
                                                    <span className="block">{t("labels.borrowDate")}: {format(new Date(record.borrowDate), "d MMM yy", { locale: dateLocale })}</span>
                                                </div>
                                                <div>
                                                    <span className="block">{t("labels.dueDate")}: {format(new Date(record.dueDate), "d MMM yy", { locale: dateLocale })}</span>
                                                </div>
                                                {record.returnDate && (
                                                    <div className="col-span-2">
                                                        <span className="text-emerald-600 font-medium">
                                                            ✓ {t("labels.returnDate")}: {format(new Date(record.returnDate), "d MMM yyyy เวลา HH:mm", { locale: dateLocale })}
                                                        </span>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex items-center gap-3 mt-2 pt-2 border-t border-[#d1cce7]/10 text-xs text-[#3d405b]/40">
                                                <span>{t("labels.rentalCoins")}: {record.rentalCoins}</span>
                                                <span>{t("labels.depositCoins")}: {record.depositCoins}</span>
                                                {record.lateFeeCoins > 0 && <span className="text-red-500">{t("labels.lateFeeCoins")}: {record.lateFeeCoins}</span>}
                                                {record.depositReturned && <span className="text-emerald-500">{t("labels.depositReturned")}</span>}
                                                {record.depositForfeited && <span className="text-red-500">{t("labels.depositForfeited")}</span>}
                                            </div>
                                        </div>
                                    );
                                })}
